...
from .pure_algebraic_four_form import FORM_KEYS, four_form_decimal_oracle, four_form_float32, four_form_float64, pure_algebraic_four_form_battery, x_grid


ROOT = Path(__file__).resolve().parents[3]
REPORT_DIR = ROOT / "Build_Docs" / "Reports" / "task028_conditional_masks_joint_lattice_pure_algebraic"
DEFAULT_OUTPUT = REPORT_DIR / "pure_algebraic_lattice_campaign_output.json"
BATTERY_OUTPUT = REPORT_DIR / "pure_algebraic_four_form_battery.json"
...
def run_campaign() -> dict[str, object]:
    values = x_grid()
    return {
        "campaign": "task028_pure_algebraic_lattice_campaign",
        "x_count": len(values),
        "x_min": values[0],
        "x_max": values[-1],
        "by_form": {
...
def write_campaign_output(path: Path = DEFAULT_OUTPUT) -> dict[str, object]:
    ...
    BATTERY_OUTPUT.write_text((json.dumps(to_json_safe(pure_algebraic_four_form_battery()), sort_keys=True, indent=2, allow_nan=False) + "\n"), encoding="utf-8")
    return payload
...
def lattice_summary_for_values(form_id: str, precision_name: str, values: Sequence[float]) -> dict[str, object]:
    ...
    for x_value in tuple(float(value) for value in values):
        form_value = _form_value(form_id, precision_name, x_value)
        if form_value == 0.0:
            continue
...
def form_value(form_id: str, precision_name: str, x_value: float) -> float:
    return _form_value(form_id, precision_name, x_value)


def _form_value(form_id: str, precision_name: str, x_value: float) -> float:
    if precision_name == "float32":
        return four_form_float32(x_value)[form_id]
    if precision_name == "float64":
        return four_form_float64(x_value)[form_id]
    return four_form_decimal_oracle(x_value, 50)[form_id]
