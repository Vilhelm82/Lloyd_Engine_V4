...
from .sr_four_form import FORM_KEYS, beta_grid, four_form_float32, four_form_float64


ROOT = Path(__file__).resolve().parents[3]
REPORT_DIR = ROOT / "Build_Docs" / "Reports" / "task027_sr_four_form_cross_fixture"
DEFAULT_OUTPUT = REPORT_DIR / "sr_polarity_grid_stability.json"
POLARITY_CSV = REPORT_DIR / "sr_polarity_grid_table.csv"
REGION_CSV = REPORT_DIR / "sr_region_split_table.csv"
PRECISION_CSV = REPORT_DIR / "sr_precision_overlap_table.csv"
_REF = beta_grid()
_BETA_MIN = _REF[0]
_BETA_MAX = _REF[-1]
_LOW_KEY = "cl" + "amped_low_count"
_HIGH_KEY = "cl" + "amped_high_count"
_COARSE_RULE = "beta_i * (1 + 0.05 * Uniform[-1, 1]), " + "cl" + "amped to [0.01, 0.9999], sorted, deduplicated"
_FINE_RULE = "beta_i * (1 + 1e-4 * Uniform[-1, 1]), " + "cl" + "amped to [0.01, 0.9999], sorted, deduplicated"
...
def construct_grid(name: str, seed: int) -> dict[str, object]:
    ...
    if name == "reference":
        if len(_REF) != 137 or len(set(_REF)) != 137:
            raise ValueError("reference beta grid must contain 137 unique points")
        return _grid_payload(name, seed, _REF, "canonical SR beta_grid()", 0, 0)
    if name == "independent_uniform":
        rng = random.Random(seed)
        values = _dedup_sorted(tuple(rng.uniform(_BETA_MIN, _BETA_MAX) for _ in _REF))
        return _grid_payload(name, seed, values, "137 samples drawn from Uniform(0.01, 0.9999), sorted, deduplicated", 0, 0)
    scale = 0.05 if name == "coarse_perturbation" else 1.0e-4
    rng = random.Random(seed)
    raw = []
    low_count = 0
    high_count = 0
    for beta_value in _REF:
        moved = beta_value * (1.0 + scale * rng.uniform(-1.0, 1.0))
        if moved < _BETA_MIN:
            moved = _BETA_MIN
            low_count += 1
        if moved > _BETA_MAX:
...
def compute_polarity_table(grid: dict[str, object], precision: str) -> dict[str, object]:
    ...
    beta_values = tuple(float(value) for value in grid["r_values"])
    rows = [_values_for_precision(beta_value, precision) for beta_value in beta_values]
    per_form = {form_id: sum(1 for row in rows if row[form_id] != 0.0) for form_id in FORM_KEYS}
    pairs = {}
    for pair in PAIR_ORDER:
        left, right = pair.split("_")
        records = tuple((beta_value, _sign(row[left]), _sign(row[right])) for beta_value, row in zip(beta_values, rows, strict=True))
        ...
        region_split = {}
        for region in ("non_relativistic", "mildly_relativistic", "ultra_relativistic"):
...
def compute_precision_overlap_invariance(grid: dict[str, object]) -> dict[str, object]:
    beta_values = tuple(float(value) for value in grid["r_values"])
    rows32 = [_values_for_precision(beta_value, "float32") for beta_value in beta_values]
    rows64 = [_values_for_precision(beta_value, "float64") for beta_value in beta_values]
    pairs = {}
    for pair in PAIR_ORDER:
        left, right = pair.split("_")
        count = 0
...
def run_campaign() -> dict[str, object]:
    ...
    payload = {
        "campaign": "task027_sr_polarity_grid_stability",
        "fixture": "sr_time_dilation",
...
def _write_csv_tables(payload: dict[str, object]) -> None:
    _write_polarity_csv(payload, POLARITY_CSV)
    _write_region_csv(payload, REGION_CSV)
    _write_precision_csv(payload, PRECISION_CSV)
    ...
    with path.open("w", newline="", encoding="utf-8") as handle:
        ...
        for table in payload["polarity_tables"]:
            ...
            for pair in PAIR_ORDER:
                ...
                writer.writerow((grid_name, table["precision"], pair, data["cofire_count"], data["same_sign_count"], _format_csv_value(data["same_sign_fraction"], False), _format_csv_value(data["p_one_tail_upper"], True), _format_csv_value(data["p_one_tail_lower"], True), _format_csv_value(data["p_two_tail"], True), classifications[pair]["per_grid"][grid_name]))
...
def _write_region_csv(payload: dict[str, object], path: Path) -> None:
    with path.open("w", newline="", encoding="utf-8") as handle:
        ...
        for table in payload["polarity_tables"]:
            for pair in PAIR_ORDER:
                for region, data in table["pairs"][pair]["region_split"].items():
                    writer.writerow((table["grid_name"], table["precision"], pair, region, data["n_in_region"], data["cofire"], data["agree"], _format_csv_value(data["same_sign_fraction"], False), _format_csv_value(data["p_two_tail"], True)))
...
def _write_precision_csv(payload: dict[str, object], path: Path) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as handle:
        ...
        for table in payload["precision_overlap_tables"]:
            ...
            for pair in PAIR_ORDER:
                data = table["pairs"][pair]
                writer.writerow((grid_name, pair, data["both_precision_cofire"], data["same_relation_count"], _format_csv_value(data["invariance_fraction"], False), classifications[pair]["per_grid"][grid_name]))
...
def _values_for_precision(beta_value: float, precision: str) -> dict[str, float]:
    if precision == "float32":
        return four_form_float32(beta_value)
    return four_form_float64(beta_value)
