...
from .schwarzschild_cbrt_four_form import FORM_KEYS, f_direct, four_form_float32, four_form_float64, schwarzschild_cbrt_four_form_battery
...
DEFAULT_OUTPUT = REPORT_DIR / "schwarzschild_cbrt_lattice_campaign_output.json"
...
def run_campaign() -> dict[str, object]:
    ...
    return {
        "campaign": "task033_schwarzschild_cbrt_lattice_campaign",
        "r_count": len(values),
        "r_min": values[0],
        "r_max": values[-1],
        "by_form": {
...
def write_campaign_output(path: Path = DEFAULT_OUTPUT) -> dict[str, object]:
    ...
    BATTERY_OUTPUT.write_text((json.dumps(to_json_safe(schwarzschild_cbrt_four_form_battery()), sort_keys=True, indent=2, allow_nan=False) + "\n"), encoding="utf-8")
    return payload
