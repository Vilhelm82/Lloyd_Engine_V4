...
from .pure_algebraic_quartic_four_form import FORM_KEYS, four_form_float32, four_form_float64, quartic_four_form_battery


ROOT = Path(__file__).resolve().parents[3]
REPORT_DIR = ROOT / "Build_Docs" / "Reports" / "task032_quartic_lattice_grain_discrimination"
DEFAULT_OUTPUT = REPORT_DIR / "quartic_lattice_campaign_output.json"
...
def run_campaign() -> dict[str, object]:
    values = x_grid()
    return {
        "campaign": "task032_quartic_lattice_campaign",
        "x_count": len(values),
        "x_min": values[0],
        "x_max": values[-1],
        "by_form": {
...
def write_campaign_output(path: Path = DEFAULT_OUTPUT) -> dict[str, object]:
    ...
    BATTERY_OUTPUT.write_text((json.dumps(to_json_safe(quartic_four_form_battery()), sort_keys=True, indent=2, allow_nan=False) + "\n"), encoding="utf-8")
    return payload
