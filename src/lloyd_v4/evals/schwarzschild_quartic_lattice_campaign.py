...
from .schwarzschild_quartic_four_form import FORM_KEYS, f_direct, four_form_float32, four_form_float64, schwarzschild_quartic_four_form_battery


ROOT = Path(__file__).resolve().parents[3]
REPORT_DIR = ROOT / "Build_Docs" / "Reports" / "task034_schwarzschild_quartic_transformed_n4"
DEFAULT_OUTPUT = REPORT_DIR / "schwarzschild_quartic_lattice_campaign_output.json"
...
def run_campaign() -> dict[str, object]:
    ...
    return {
        "campaign": "task034_schwarzschild_quartic_lattice_campaign",
        "r_count": len(values),
        "r_min": values[0],
        "r_max": values[-1],
        "by_form": {
...
def write_campaign_output(path: Path = DEFAULT_OUTPUT) -> dict[str, object]:
    ...
    BATTERY_OUTPUT.write_text((json.dumps(to_json_safe(schwarzschild_quartic_four_form_battery()), sort_keys=True, indent=2, allow_nan=False) + "\n"), encoding="utf-8")
    return payload
...
def lattice_summary_for_values(form_id: str, precision_name: str, values: Sequence[float]) -> dict[str, object]:
    ...
    for r_value in tuple(float(value) for value in values):
        form_value = form_value_for_precision(form_id, precision_name, r_value)
        if form_value == 0.0:
            continue
        unit_value = unit_for_precision(r_value, precision_name)
        ...
        regions[_region_name(r_value)].append(rounded_level)
...
def unit_for_precision(r_value: float, precision_name: str) -> float:
    if precision_name == "float32":
        s = np.float32
        direct = s(1.0) - s(2.0) / s(r_value)
        return _ulp_of_float32(float(direct))
    return ulp_of_double(f_direct(r_value))


def form_value_for_precision(form_id: str, precision_name: str, r_value: float) -> float:
    if precision_name == "float32":
