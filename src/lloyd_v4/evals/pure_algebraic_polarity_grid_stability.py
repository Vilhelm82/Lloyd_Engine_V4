...
from .pure_algebraic_four_form import FORM_KEYS, four_form_float32, four_form_float64, x_grid


ROOT = Path(__file__).resolve().parents[3]
REPORT_DIR = ROOT / "Build_Docs" / "Reports" / "task028_conditional_masks_joint_lattice_pure_algebraic"
DEFAULT_OUTPUT = REPORT_DIR / "pure_algebraic_polarity_grid_stability.json"
POLARITY_CSV = REPORT_DIR / "pure_algebraic_polarity_grid_table.csv"
REGION_CSV = REPORT_DIR / "pure_algebraic_region_split_table.csv"
PRECISION_CSV = REPORT_DIR / "pure_algebraic_precision_overlap_table.csv"
...
_COARSE_RULE = "x_i * (1 + 0.05 * Uniform[-1, 1]), " + "cl" + "amped to [0.01, 0.99], sorted, deduplicated"
_FINE_RULE = "x_i * (1 + 1e-4 * Uniform[-1, 1]), " + "cl" + "amped to [0.01, 0.99], sorted, deduplicated"
...
def construct_grid(name: str, seed: int) -> dict[str, object]:
    ...
    if name == "reference":
        if len(_REF) != 137 or len(set(_REF)) != 137:
            raise ValueError("reference x grid must contain 137 unique points")
        return _grid_payload(name, seed, _REF, "canonical pure algebraic x_grid()", 0, 0)
    if name == "independent_uniform":
        rng = random.Random(seed)
        values = _dedup_sorted(tuple(rng.uniform(_X_MIN, _X_MAX) for _ in _REF))
        return _grid_payload(name, seed, values, "137 samples drawn from Uniform(0.01, 0.99), sorted, deduplicated", 0, 0)
    ...
    high_count = 0
    for x_value in _REF:
        moved = x_value * (1.0 + scale * rng.uniform(-1.0, 1.0))
        if moved < _X_MIN:
            moved = _X_MIN
            low_count += 1
        if moved > _X_MAX:
...
def compute_polarity_table(grid: dict[str, object], precision: str) -> dict[str, object]:
    ...
    x_values = tuple(float(value) for value in grid["r_values"])
    rows = [_values_for_precision(x_value, precision) for x_value in x_values]
    per_form = {form_id: sum(1 for row in rows if row[form_id] != 0.0) for form_id in FORM_KEYS}
    pairs = {}
    for pair in PAIR_ORDER:
        left, right = pair.split("_")
        records = tuple((x_value, _sign(row[left]), _sign(row[right])) for x_value, row in zip(x_values, rows, strict=True))
...
def compute_precision_overlap_invariance(grid: dict[str, object]) -> dict[str, object]:
    x_values = tuple(float(value) for value in grid["r_values"])
    rows32 = [_values_for_precision(x_value, "float32") for x_value in x_values]
    rows64 = [_values_for_precision(x_value, "float64") for x_value in x_values]
    pairs = {}
    for pair in PAIR_ORDER:
        left, right = pair.split("_")
        count = 0
...
def run_campaign() -> dict[str, object]:
    ...
    return {
        "campaign": "task028_pure_algebraic_polarity_grid_stability",
        "fixture": "pure_algebraic",
...
def _values_for_precision(x_value: float, precision: str) -> dict[str, float]:
    if precision == "float32":
        return four_form_float32(x_value)
    return four_form_float64(x_value)
